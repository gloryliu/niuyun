 ...
 */
public class IndexLiveListItemAdapter2 extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private List<DianboListBean.DataBeanX.DataBean> list;
    ...
    public IndexLiveListItemAdapter2(Context context) {
        this.context = context;
        this.list = new ArrayList<>();
        mLayoutInflater = LayoutInflater.from(context);
    }

    public IndexLiveListItemAdapter2(Context context, List<DianboListBean.DataBeanX.DataBean> items) {
        this.context = context;
        this.list = new ArrayList<>();
    ...
    public void addList(List<DianboListBean.DataBeanX.DataBean> items) {
        this.list.addAll(items);
    ...
    public List<DianboListBean.DataBeanX.DataBean> getEntities() {
        return list;
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new ImageViewHolder(mLayoutInflater.inflate(R.layout.fragment_index_live_list_item2, parent, false));
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, final int position) {
        if (list != null) {
            ...
            ((ImageViewHolder) viewHolder).tv_name.setText(list.get(position).getTitle());
//            ((ImageViewHolder) viewHolder).tv_company_name.setText(list.get(position).getName());
            ((ImageViewHolder) viewHolder).tv_describe.setText(list.get(position).getDescribe());
...
//            }
            ImageLoadedrManager.getInstance().display(context, Constants.COMMON_cover_URL + list.get(position).getImageUrl(), ((ImageViewHolder) viewHolder).iv_live_cover);
