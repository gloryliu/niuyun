...
import com.niuyun.hire.ui.chat.model.MessageFactory;
...
public class PushUtil implements Observer {
    ...
    private void PushNotify(TIMMessage msg){
        ...
        NotificationManager mNotificationManager = (NotificationManager) BaseContext.getInstance().getSystemService(BaseContext.getInstance().NOTIFICATION_SERVICE);
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(BaseContext.getInstance());
        Intent notificationIntent = new Intent(BaseContext.getInstance(), HomeActivity.class);
        notificationIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP
                | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent intent = PendingIntent.getActivity(BaseContext.getInstance(), 0,
...
//                .setNumber(++pushNum) //设置通知集合的数量
    ...
    public void reset(){
        NotificationManager notificationManager = (NotificationManager)BaseContext.getInstance().getSystemService(BaseContext.getInstance().NOTIFICATION_SERVICE);
