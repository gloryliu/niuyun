package com.niuyun.hire.ui.polyvLive.danmaku;
...
public class RelativeImageSpan extends ImageSpan {
    ...
    private int paddingleft = -1, paddingright = -1;
    ...
    public RelativeImageSpan(Drawable d, int verticalAlignment, int paddingleft, int paddingright) {
        super(d, verticalAlignment);
        status = verticalAlignment;
    ...
    public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom,
        ...
        // 偏移画布后开始绘制
        if (paddingleft != -1)
            canvas.translate(x + paddingleft, transY);
        else
    ...
    public int getSize(Paint paint, CharSequence text, int start, int end, FontMetricsInt fm) {
        ...
        return paddingright == -1 ? rect.right + ((int) (3 * scale * 2)) : rect.right + paddingright + paddingleft;
    }
}
